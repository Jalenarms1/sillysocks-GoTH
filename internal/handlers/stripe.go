...
func HandleCreateCheckoutSession(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Println(total)
	fmt.Println(float64(total/100) * 1.08)
...
func HandleCheckoutSessionWebhook(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		http.Error(w, "Not authorized to access the endpoint requested", http.StatusBadRequest)
		return
	}
	...
	if event.Type == stripe.EventType(stripe.EventTypeCheckoutSessionCompleted) {
		...
		if status == stripe.CheckoutSessionPaymentStatusPaid {
